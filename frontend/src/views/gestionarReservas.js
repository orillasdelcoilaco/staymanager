...
import { renderTabla } from './components/gestionarReservas/reservas.table.js';
import { 
    renderDocumentoLink,
    abrirModalVer,
    abrirModalEditar,
...
export function afterRender() {
    ...
    renderTabla(getFiltros(), todasLasReservas, historialCargas);

    [searchInput, cargaFilter, canalFilter, estadoFilter, estadoGestionFilter, fechaInicioFilter, fechaFinFilter].forEach(el => {
        el.addEventListener('input', () => renderTabla(getFiltros(), todasLasReservas, historialCargas));
    });
    ...
    if (formEdit) {
        ...
        formEdit.addEventListener('change', async e => {
            if (e.target.classList.contains('doc-input')) {
                const updatedReserva = await handleGestionarDocumento(editandoReserva.id, e.target.dataset.tipo, e.target.files[0], 'upload');
                if (updatedReserva) {
                    editandoReserva = updatedReserva;
                }
            }
        });
        formEdit.addEventListener('click', async e => {
            if (e.target.classList.contains('delete-doc-btn')) {
                if (confirm('¿Seguro que quieres eliminar este documento?')) {
                    const updatedReserva = await handleGestionarDocumento(editandoReserva.id, e.target.dataset.tipo, null, 'delete');
                    if (updatedReserva) {
                        editandoReserva = updatedReserva;
                    }
                }
    ...
    if (reservaIdParaEditar) {
        abrirModalEditar(reservaIdParaEditar, alojamientos, clientes).then(reserva => {
