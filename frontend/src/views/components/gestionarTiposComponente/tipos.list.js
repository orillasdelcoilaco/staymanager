...
export const renderTablaTipos = (tipos) => {
    ...
    return categoriasOrdenadas.map((categoria, index) => {
        ...
        const itemRows = items.map(t => {
            const nombre = t.nombreNormalizado || '<span class="text-red-400 italic">Sin Nombre</span>';
            const icono = t.icono || '📦';
            ...
            <tr class="border-b hover:bg-gray-50 transition-colors group-item-${groupId}">
                ...
                <td class="py-3 px-4 text-right">
                    <button data-id="${t.id}" class="btn-edit-tipo text-indigo-600 hover:text-indigo-800 p-2 rounded hover:bg-indigo-50 mr-1" title="Editar">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                    </button>
                    <button data-id="${t.id}" class="btn-delete-tipo text-red-500 hover:text-red-700 p-2 rounded hover:bg-red-50" title="Eliminar">
