...
module.exports = (db) => {
    ...
    router.get('/propiedad/:propiedadId/photo-plan', async (req, res, next) => {
        try {
            ...
            const [tipos, tiposElemento] = await Promise.all([
                obtenerTiposPorEmpresa(db, empresaId),
                obtenerTiposElemento(db, empresaId)
            ]);

            const plan = generarPlanFotos(propiedad.componentes, tipos, tiposElemento);
    ...
    router.post('/propiedad/:propiedadId/generate-ai-text', async (req, res, next) => {
        try {
            ...
            const descripcion = await generarDescripcionAlojamiento(
            ...
            res.status(200).json({ texto: descripcion });
        } catch (error) {
            next(error);
        }
    });
