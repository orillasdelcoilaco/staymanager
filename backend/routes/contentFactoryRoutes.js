// backend/routes/contentFactoryRoutes.js
const express = require('express');
const multer = require('multer');
const { optimizarPerfilAlojamiento, analizarRequisitosFotos, auditarFoto } = require('../services/contentFactoryService');
const { optimizeImage } = require('../services/imageProcessingService'); // Importar servicio de optimización
const { obtenerPropiedadPorId } = require('../services/propiedadesService'); // Para validar existencia básica
...
module.exports = (db) => {
    ...
    router.post('/photos/audit', upload.single('photo'), async (req, res) => {
        try {
            const { empresaId } = req.user;
            const { alojamientoId, componenteId, tipoComponente, nombreComponente } = req.body;
            ...
            const nombreAlojamiento = alojamiento ? alojamiento.nombre : "Alojamiento";
            ...
            // Retornamos el resultado IA junto con metadatos técnicos de optimización
            // NOTA: En un flujo real, aquí devolveríamos el buffer o una URL temporal si ya se guardó.
            // Para "audit", devolvemos la info para que el frontend sepa que se procesó bien.
            res.status(200).json({
                ...resultadoIA,
                technicalMetadata: {
