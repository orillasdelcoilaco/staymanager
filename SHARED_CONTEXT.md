...
>
> **Dominios, host de API y nombres reales (handoffs, OpenAPI, prompts a Claude Code):** no inventar `ejemplo.com` o `TU-SERVICIO`; usar la tabla en **`LEER-PRIMERO.md`** (sección **Referencias de entorno**). No sustituye el producto multi-tenant: en código se sigue `empresa_id` y parametría; la tabla es solo **documentación operativa** canónica.
>
...
>
> **IA de venta (ChatGPT / OpenAPI / reserva pública) con Cursor en paralelo:** además de este archivo, leer `TASKS/backlog-producto-pendientes.md` (roadmap), `**TASKS/tema/SM-venta-ia/venta-ia.md`** (canales externos, MCP, Google Hotels/Travel, checklist unificado) y `TASKS/coordinacion-cursor-claude-ia-venta.md` (avisos cruzados). El producto **es** el canal de venta principal del operador; **no** se persiguen **OTAs de terceros como canal de venta** (subir inventario para vender ahí). **Google Hotel Center** como **connectivity partner** de la plataforma (feeds agregados bajo el dominio de plataforma **`rezerva.cl`**, deep links al checkout del tenant) es descubrimiento y cumplimiento de políticas Google, **no** reventa vía agregador — detalle `**TASKS/tema/SM-venta-ia/venta-ia.md` §1.1 y §7**. **Sí** puede haber **conexión operativa** (iCal, reportes, importación de reservas) para unificar calendario — eso no es “venta por OTA” y sigue definido en el resto del sistema. El detalle del procedimiento para Claude Code está en `CLAUDE.md` (sección *Antigravity / IA de venta*).
>
> **Refactor en curso — identificadores vs nombres (2026-04-24):** no introducir nuevas dependencias de **nombres/etiquetas que el usuario puede cambiar** (p. ej. `estados_reserva.nombre`, `canales.nombre`, nombre de plantilla) como clave en SQL, comparaciones de negocio o payloads API. Usar **id**, `**semantica`** (donde ya exista) o claves de producto acordadas; el nombre solo para UI. Inventario y checklist: `TASKS/tema/SM-ids-vs-names/audit-identificadores-vs-nombres-ui.md`.
>
> **Ubicación de archivos (todos los agentes, 2026-05-06):** no crear artefactos nuevos en la **raíz del repositorio** ni en carpetas improvisadas (`docs/`, `temp/`, etc.). Seguir el mapa en **`LEER-PRIMERO.md`** (tabla *Estándares* → fila **Ubicación de archivos**), **`CLAUDE.md`** (sección *Flujo de Trabajo y Comandos* ampliada) y la regla Cursor **`07-artifact-placement-repo-layout.mdc`**. Documentación por iniciativa va en **`TASKS/tema/<SM-id>/`** alineado a **`TASKS/tablero.md`**.
>
> **Tablero y sesiones (2026-05-06):** flujo **contexto → tema → qué hacer** en **`LEER-PRIMERO.md`**; al **crear, modificar o eliminar** trabajo perteneciente a una iniciativa, **actualizar la fila** en **`TASKS/tablero.md`** (fecha y nota). Regla **`50-tasks-tablero-y-temas.mdc`**.
...
| Fecha      | Autor       | Cambio                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-05-07 | Cursor      | **Dominio plataforma canónico `rezerva.cl`:** tabla **Referencias de entorno** en `LEER-PRIMERO.md`; `TASKS/tema/SM-venta-ia/venta-ia.md` §1.2 + guías smoke (`gemini-smoke-instrucciones.md`, `qa-y-seguimiento-prelaunch-canales.md`); cabecera **SHARED_CONTEXT** (connectivity partner). API/host Render **`suite-manager.onrender.com`** sin cambio.                                                                 |
| 2026-05-06 | Cursor      | **Referencias de entorno** en `LEER-PRIMERO.md` (dominio `suitemanagers.com`, API `suite-manager.onrender.com`, GPT **SuiteManager Marketplace IA**, tenant ejemplo documental `orillasdelcoilaco`); cabecera **SHARED_CONTEXT** enlaza la sección.                                                                                                                                                                  |
| 2026-05-02 | Cursor      | `**LEER-PRIMERO.md**`: tabla **Estándares de implementación** + lectura obligatoria de `CLAUDE.md` también en Cursor; `**TASKS/tema/SM-venta-ia/venta-ia.md`**: automatización producto + venta vs sync operativa (sin OTAs como canal de venta); multi-agente Cursor: `**TASKS/coordinacion-cursor-paralelo.md**` (paso 1 = raíz `LEER-PRIMERO.md`). **Misma fecha:** Google Hotels — `**TASKS/tema/SM-venta-ia/venta-ia.md` §7.9–§7.11** (Go PR: agregador + XSD, paridad SSR, DNS TTL). |
| 2026-05-01 | Cursor      | Roadmap venta IA + canales externos unificado en `TASKS/tema/SM-venta-ia/venta-ia.md`; backlog §5.3 enlaza allí; lectura conjunta con coordinación IA venta. **Actualización misma fecha:** panel SPA **Operaciones → Canales IA** (`/canales-ia`) como entrada usuario única para tokens feeds Google Hotels/ARI, semáforo y `hotelId`/listado por alojamiento — ver `**TASKS/tema/SM-venta-ia/venta-ia.md` §2.6**.                                 |
...
| 2026-04-24 | Cursor      | Implementación fase 1: `reservas.estado_gestion_id` (migración `backend/db/migrations/reservas-estado-gestion-id.sql`), servicios y SPA parcial; checklist actualizado en `TASKS/tema/SM-ids-vs-names/audit-identificadores-vs-nombres-ui.md`                                                                                                                                                                                           |
| 2026-04-24 | Cursor      | Aviso global + regla §3: catálogos por id/semántica; checklist `TASKS/tema/SM-ids-vs-names/audit-identificadores-vs-nombres-ui.md`                                                                                                                                                                                                                                                                                                      |
...
- **Principal**: Definido en `TASKS/tema/SM-migracion-postgres/migration-plan-postgres.md` (sección 3)
...
- **Scripts** que aún tocan `empresas/.../reservas` en Firestore (p. ej. `backend/scripts/backfill-reservas-firestore-estados-ids.js`, `backend/scripts/verify_property_migration_result.js`, `scripts/legacy/borrar-historico-prueba.js`) son **solo migración o limpieza legacy**, no el flujo operativo del SaaS.
...
- 📄 **Checklist por archivo:** `TASKS/tema/SM-ids-vs-names/audit-identificadores-vs-nombres-ui.md`
...
```bash
node scripts/tooling/audit-complexity.js    # Después de TODO cambio de código
node scripts/tooling/audit-ui.js            # Después de cambios frontend
```
...
│
├── TASKS/
│   ├── tema/SM-migracion-postgres/migration-plan-postgres.md
│   ├── tema/SM-operacion-agentes/pending.md, completed.md
│   └── (backlog, venta-ia, coordinacion-cursor-*.md, tablero en raíz TASKS/; `LEER-PRIMERO.md` en raíz repo)
│
└── scripts/
    ├── README.md              ← índice (tooling + legacy)
    ├── tooling/               ← auditorías, créditos, migrate-colors, hooks
    └── legacy/                ← scripts one-off / migraciones puntuales
```
...
5. **Acuerdo Arquitectura IA (Escalera de Costos Gemma)**: Tras revisar el análisis crítico, emití la contrapropuesta final [acuerdo-arquitectura-ia.md](file:///d:/pmeza/Desarrollos%20Render/staymanager/TASKS/tema/SM-ia-arquitectura/acuerdo-arquitectura-ia.md). **Claude Code:** El diseño técnico oficial es ahora un "Strategy Pattern basado en Enums de Tareas". Revisa el plan de Sprints e inicia el Sprint 1 de inmediato.
