 ...
 *
 * Uso:
 *   node scripts/tooling/check-new-criticos.js           → compara contra baseline
 *   node scripts/tooling/check-new-criticos.js --update  → guarda el estado actual como nuevo baseline
 *
 * Exit codes:
 *   0 = sin críticos nuevos (o actualización exitosa)
 *   1 = hay críticos nuevos → bloquea el pre-push hook
 *
 * El baseline se guarda en TASKS/tema/SM-auditoria-calidad/complexity-baseline.json
 */
...
if (UPDATE_MODE) {
    ...
    fs.writeFileSync(BASELINE_PATH, JSON.stringify({
        ...
        console.log(
            `\n✅ Baseline actualizado: ${actuales.length} críticos guardados en TASKS/tema/SM-auditoria-calidad/complexity-baseline.json\n`
        );
    process.exit(0);
}
...
if (!fs.existsSync(BASELINE_PATH)) {
    console.log('\n⚠️  No existe baseline. Ejecuta primero:\n   node scripts/tooling/check-new-criticos.js --update\n');
    process.exit(0);
}
...
console.log(`  node scripts/tooling/check-new-criticos.js --update\n`);
