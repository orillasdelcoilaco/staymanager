...
function ejecutarAuditoria() {
    ...
    try {
        ...
        execSync('node scripts/tooling/audit-complexity.js', {
            encoding: 'utf8',
            stdio: 'inherit',
            cwd: process.cwd()
        });
        ...
        console.log('\n' + '=' .repeat(60));
        console.log('📎 Rutas scripts/ en documentación (.md / .mdc)');
        console.log('=' .repeat(60) + '\n');

        execSync('node backend/scripts/test-repo-root-scripts-paths-doc.js', {
            encoding: 'utf8',
            stdio: 'inherit',
            cwd: process.cwd()
        });
