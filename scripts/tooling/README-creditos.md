...
node scripts/tooling/hooks-creditos.js audit-ui

# Auditoría de complejidad
node scripts/tooling/hooks-creditos.js audit-complexity
```

### 3. Registrar operaciones manualmente:
```bash
# Registrar una operación simple
node scripts/tooling/monitor-creditos.js operacion general 1.0

# Registrar una auditoría (más compleja)
node scripts/tooling/monitor-creditos.js operacion auditoria 2.5
```
...
const { CreditosHooks } = require('./scripts/tooling/hooks-creditos.js');
...
| `audit-complexity.js` | `hooks-creditos.js audit-complexity` | Tras **`audit-complexity.js`**, ejecuta **`backend/scripts/test-repo-root-scripts-paths-doc.js`** (una sola vez por sesión si también corres UI; no está duplicado en `audit-ui`) |
| `migrate-colors.js` | `hooks-creditos.js migrate-colors` | Verifica créditos antes de ejecutar |

## ⚙️ Configuración

### Archivo de configuración: `scripts/tooling/config-creditos.json`
...
cp scripts/tooling/config-creditos.json ~/.claude/credit-monitor.json
...
2. **Archivo**: `TASKS/tema/SM-operacion-agentes/alertas-creditos.md` (historial)
...
1. **Antes de tarea larga**:
   ```bash
   node scripts/tooling/hooks-creditos.js reporte-creditos
   ```
...
3. **Verificar resultado**:
   ```bash
   cat TASKS/tema/SM-operacion-agentes/alertas-creditos.md | tail -20
   ```
...
```bash
node scripts/tooling/monitor-creditos.js reporte
node scripts/tooling/monitor-creditos.js operacion <tipo> <complejidad>
node scripts/tooling/monitor-creditos.js finalizar
node scripts/tooling/monitor-creditos.js alerta-test
```
...
node scripts/tooling/hooks-creditos.js audit-ui
node scripts/tooling/hooks-creditos.js audit-complexity
node scripts/tooling/hooks-creditos.js migrate-colors
node scripts/tooling/hooks-creditos.js build-css
node scripts/tooling/hooks-creditos.js reporte-creditos
```
