...
const MonitorCreditos = require(path.join(__dirname, 'monitor-creditos.js'));
        ...
        const scripts = {
            'audit-ui': () => {
                if (this.preAuditoria('UI')) {
                    const inicio = Date.now();
                    const resultado = this.ejecutarConMonitoreo('scripts/tooling/audit-ui.js');
                    const duracion = (Date.now() - inicio) / 1000;
            ...
            'audit-complexity': () => {
                if (this.preAuditoria('COMPLEJIDAD')) {
                    const inicio = Date.now();
                    let resultado = this.ejecutarConMonitoreo('scripts/tooling/audit-complexity.js');
                    if (resultado.exito) {
                        resultado = this.ejecutarConMonitoreo('backend/scripts/test-repo-root-scripts-paths-doc.js');
                    }
                    const duracion = (Date.now() - inicio) / 1000;
            ...
            'migrate-colors': () => {
                ...
                if (this.verificarAntesDeTareaLarga('Migración de Colores', 0.5)) {
                    this.ejecutarConMonitoreo('scripts/tooling/migrate-colors.js');
                }
            },
...
if (require.main === module) {
    ...
    if (scripts[comando]) {
        ...
        console.log('Uso: node scripts/tooling/hooks-creditos.js [comando]');
        ...
        console.log('\nEjemplo:');
        console.log('  node scripts/tooling/hooks-creditos.js audit-ui');
    }
}
