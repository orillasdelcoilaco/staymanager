...
2. **`npm run test:ssr`** con el **servidor SSR en marcha** (`localhost:3001` y URL de prueba configurada en el script) si quieres validar HTTP vivo tras tocar vistas/rutas públicas. *(Sigue siendo opcional para el tag; no sustituye el smoke manual §2.3.)*
