...
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + suitemanagers.com) | En curso (Cursor) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor | 2026-05-06 — **Storage rules en repo** `backend/firebase/storage.rules` (lectura `empresas/**`); deploy: `firebase deploy --only storage` desde `backend/firebase` | `backend/firebase/*`, README tema |
