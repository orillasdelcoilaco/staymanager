...
|----|---------------|---------|------|--------------|-------------|---------------------|---------|
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + suitemanagers.com) | En curso (Cursor) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor | 2026-05-05 — PSI follow-up: thumb `_thumb.webp` derivado + srcset tarjetas; CSS `--minify`; a11y concierge/footer/listado; `website.css` minificado | `tema/SM-pagespeed-lighthouse-max/README.md`, `audit-causas-raiz.md` |
...
| `SM-ssr-sitio-publico` | SSR / sitio público (arquitectura, empresa, wizard contenido) | Listo | — | `tema/SM-ssr-sitio-publico` | — | 2026-05-05 — Home `home.ejs`: logo solo en header; LCP (preload/preconnect/dimensiones/imagen); Font Awesome no bloqueante; CLS imgs tarjetas; a11y iconos/header | `tema/SM-ssr-sitio-publico/plan-ssr-arquitectura-dos-capas.md`, `plan-accion-ssr-empresa.md`, `plan-contenido-web-wizard.md` |
...
| `SM-fotos-galeria` | Fotos, galería, IMG-001, baños | Listo | — | `tema/SM-fotos-galeria` | — | 2026-05-05 — Validación estricta full+thumb (`imageUploadGuards`); limpieza datos legacy: `node scripts/tooling/cleanup-invalid-web-images.js --subdomain=…` (dry-run) luego `--apply` | `galeriaService.js`, `config.routes.js`, `cleanup-invalid-web-images.js` |
