...
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + suitemanagers.com) | En curso (Cursor) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor | 2026-05-06 — Miniaturas/hero: flujo único en **Contenido Web → Fotos → Continuar con SEO** (`POST /api/website/propiedad/:id/prepare-for-seo-step` → `runWebImagesRepair` acotado + hero empresa + `syncToWebsite` + invalidación caché); WhatsApp success-700; img `onerror`; hero thumb q58 | `websiteAlojamientos.js`, `webImagesRepairApi.js`, `webImagesRepairService.js`, EJS cards/home |
