...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-07 — Repo canónico **`admin-rezerva/rezerva`** (README, TEAM_CONFIG, plan §4.1); **`git remote`** → ese URL; commits de migración en **main** listos; **push** manual cuando exista el repo en GitHub. Resto pendiente: Render reconnect, Fase 1/3 ops, Fase 4 PG, `verify-subdomain-logic` | `tema/SM-rebrand-dominio/pendientes-y-registro.md`, `plan-accion-rebrand.md`, `README.md` |
