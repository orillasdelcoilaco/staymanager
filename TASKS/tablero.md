...
| `SM-npm-audit-zero` | npm audit → 0 (build Render / dependencias backend) | En curso (Cursor) | — | `tema/SM-npm-audit-zero` | Cursor | 2026-05-06 — `npm audit fix` en raíz → 0 CVE; en `backend/` quedan 13 (xlsx sin fix; resto pide `audit fix --force` o subir deps a mano); `test:ci` OK; pendiente commit + fase 2 cierre de 13 | `tema/SM-npm-audit-zero/README.md`, `plan-accion-npm-audit-zero.md` |
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + suitemanagers.com) | En curso (Cursor) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor | 2026-05-06 — Thumbs listado/galería 560px q66; hero thumb 900px q64; `repair-web-images` alineado; property-card CLS 560; contrastes texto tarjeta | `galeriaService.js`, `config.routes.js`, `repair-web-images.js`, `property-card.ejs`, `home.ejs` |
...
| `SM-ssr-sitio-publico` | SSR / sitio público (arquitectura, empresa, wizard contenido) | Listo | — | `tema/SM-ssr-sitio-publico` | — | 2026-05-06 — `POST /api/website/optimize-profile`: errores devuelven JSON (antes `next(error)` → HTML y `fetchAPI` fallaba); 503 si IA vacía; 429 cuota | `config.routes.js` (`optimize-profile`) |
...
| `SM-fotos-galeria` | Fotos, galería, IMG-001, baños | Listo | — | `tema/SM-fotos-galeria` | Cursor | 2026-05-06 — Panel: `POST /api/website/maintenance/regenerate-web-images` + sección en Config. Web; servicio `webImagesRepairService.js` compartido con `repair-web-images.js` (CLI) | `webImagesRepairApi.js`, `webImagesRepairService.js`, `webPublica.general.js` |
