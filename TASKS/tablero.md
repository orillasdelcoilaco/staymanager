...
|----|---------------|---------|------|--------------|-------------|---------------------|---------|
| `SM-blog-publico` | Blog SSR por empresa (sugerencias datos + IA + panel) | Listo | — | `tema/SM-blog-publico` | Cursor | 2026-05-06 — Fix deploy Render: archivos blog (`websiteBlogApi`, servicios, vistas, migración) subidos a main — antes MODULE_NOT_FOUND | `routes/websiteBlogApi.js`, `blogPostService.js` |
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + suitemanagers.com) | En curso (Cursor) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor | 2026-05-06 — Home SSR: tarjetas solo miniatura (sin srcset al full ~1.7MB); h2 visible listado + `aria-labelledby`; `h3` nombre alojamiento; footer contraste; concierge `title` + `aria-label` | `property-card.ejs`, `home.ejs`, `footer.ejs`, `chat-widget.ejs`, `README.md` |
