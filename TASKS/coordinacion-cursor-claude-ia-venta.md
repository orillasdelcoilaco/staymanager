...
|-------------|--------|----------------------------------|----------|----------------------------------|
| 2026-04-24  | Cursor | Detalle IA `precio_estimado` con fechas | LISTO | `publicAiPrecioEstimadoService.js`, `publicAiHttpController.detalle`, OpenAPI 1.4.0. |
...
| 2026-04-24  | Cursor | Payload comercial IA (listado/detalle) | LISTO | `publicAiMarketingLayer.js`, `publicAiProductSnapshot.js`, `publicAiHttpController` (galería `espacio`). Sin cambio de rutas HTTP. |
...
| 2026-04-24  | Cursor | `GET /api/disponibilidad` enriquecida + vibe búsqueda + confirmación reserva | LISTO | `publicAiDisponibilidadService.js`, `evaluarRestriccionesReservaWebCodigo`, `publicAiHttpController`, OpenAPI; `requiere_confirmacion_final` en detalle; validación email/tel `publicAiController`. |
...
| OpenAPI / GPT | `backend/openapi/openapi-chatgpt.yaml`, `publicAiHttpController.js`, `publicRoutes.js` | El agente llama endpoints rotos o con body distinto |
...
- 2026-05-07 — Cursor — **MCP — documentación obligatoria para integradores:** nombre npm **`rezerva-mcp-server`** (antes `suitemanager-mcp-server`); punteros canónicos **`LEER-PRIMERO.md`** (*Referencias de entorno*, fila MCP + callout), **`backend/ai/openai/mcp-server/README.md`**, **`TASKS/tema/SM-venta-ia/chatgpt_integration_summary.md`** (cabecera y §2.B); **`venta-ia.md`** §2.4 — actualizar JSON de Claude Desktop / Cursor / scripts que arranquen el paquete viejo.
- 2026-05-07 — Cursor — **`publicAiHttpController.js`** (ex `suitemanagerApiController.js`) + MCP **`rezerva-mcp-server`**; imports en `routes/api.js`, `agentes.js`, `publicRoutes.js`; rutas HTTP sin cambio.
- 2026-05-07 — Cursor — **`SM-rebrand-dominio`:** barrido scripts smokes/audit + docs TASKS (GHC checklist, SEO, blog, pagespeed, etc.); `pendientes-y-registro.md`: **pendiente** ops DNS/repo, Fase 4 PG (rename controller **hecho**).
- 2026-05-07 — Cursor — **`SM-rebrand-dominio`:** creado **`TASKS/tema/SM-rebrand-dominio/pendientes-y-registro.md`** — ops Fase 0/1/3 no ejecutadas, docs secundarios en otros temas, DoD grep/scripts, intencional `config.routes.js`; tablero/README/plan enlazan ahí.
- 2026-05-07 — Cursor — **Rebrand dominio (documentación):** `LEER-PRIMERO.md` tabla hosts, `SHARED_CONTEXT`, `venta-ia.md` §1.2 + inventario, `gemini-smoke-instrucciones.md`, `qa-y-seguimiento-prelaunch-canales.md`, `SM-seo-ssr-buscadores/README.md` → **`rezerva.cl`** canónico; API sigue `suite-manager.onrender.com`; sin cambio contrato OpenAPI ni rutas agente.
