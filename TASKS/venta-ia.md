...
| **Address** | Tags desglosados: calle, ciudad, país, código postal (según XSD Google / validación interna). Si `tipoNegocio` es **`complejo`** o **`hotel`**, la dirección puede tomarse de **`configuracion.ubicacion`** / contacto web de la empresa cuando falte en cada `googleHotelData.address` (`googleHotelsEmpresaAddress.js` — health + feeds tenant y global). **`cartera`:** sigue exigiendo dirección por alojamiento. |
